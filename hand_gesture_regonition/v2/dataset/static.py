...
DATASET_PATH = Path("data/v2/static/dataset.csv")
IMAGE_DIR = Path("data/v2/static/")
...
def generate():
    """
    Generates a DataFrame with landmark data from images.
    The DataFrame will have columns: name, gesture, and landmark columns.
    """
    ...
    for gesture_path in IMAGE_DIR.iterdir():
        if gesture_path.is_dir():
            ...
            for image_path in gesture_path.iterdir():
                if image_path.suffix.lower() in ('.png', '.jpg', '.jpeg'):
                    ...
                    if results.multi_hand_landmarks and results.multi_handedness:
                        for i, hand_landmarks in enumerate(results.multi_hand_landmarks):
                            ...
                            if process_hand:
                                row = {'name': filename, 'gesture': gesture_name}
